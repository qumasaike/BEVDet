...
@HEADS.register_module()
class Anchor3DHeadTRT(Anchor3DHead):
    ...
    def __init__(self,
                ...
                # voxel_size,
                **kwards):
        super().__init__(anchor_generator=anchor_generator,**kwards)
        ranges = anchor_generator['ranges'][0]
        feature_size =  round((ranges[4] - ranges[1]) / voxel_size[1]), round((ranges[3] - ranges[0]) / voxel_size[0])
        featmap_sizes = [torch.Size(feature_size)]
        self.mlvl_anchors = self.anchor_generator.grid_anchors(
            featmap_sizes)
        self.mlvl_anchors = [
            anchor.reshape(-1, self.box_code_size) for anchor in self.mlvl_anchors
        ]
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(input_metas)):
            ...
            input_meta = input_metas[img_id]
            proposals = self.get_bboxes_single(cls_score_list, bbox_pred_list,
                                               dir_cls_pred_list, self.mlvl_anchors,
                                               input_meta, cfg, rescale)
            result_list.append(proposals)
        return result_list        
    ...
    def get_bboxes_single(self,
        ...
        score_thr = cfg.get('score_thr', 0)
        scores, labels = torch.max(mlvl_scores,1)

        # scores, indices = torch.topk(mlvl_scores[:,:-1], k=100, dim=0)
        # bboxes = mlvl_bboxes[indices][:,:,:7]
        return mlvl_bboxes, scores,labels.float()
