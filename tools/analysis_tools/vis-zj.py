...
def lidar2img(points_lidar, camrera_info):
    ...
    camera2lidar[:3, :3] = Quaternion(camrera_info['sensor2lidar_rotation']).rotation_matrix
...
def get_lidar2global(infos):
    ...
    lidar2ego[:3, 3] = infos['lidar2ego_translation'][:3]
...
def parse_args():
    parser = argparse.ArgumentParser(description='Visualize the predicted '
                                     'result of nuScenes')
    parser.add_argument(
        '--res', default="trainzjdetcbg/pts_bbox/results_nusc.json",help='Path to the predicted result in json format')
    parser.add_argument(
        '--show-range',
        type=int,
        default=50,
    ...
    parser.add_argument(
        '--canva-size', type=int, default=10, help='Size of canva in pixel')
    parser.add_argument(
        '--vis-frames',
        type=int,
        default=500,
    ...
    parser.add_argument('--draw-gt', type=bool, default=False)
    parser.add_argument(
        '--version',
        type=str,
        default='val',
...
def main():
    ...
    for cnt, infos in enumerate(
            dataset['infos'][:len(dataset['infos'])]):
        # if '1555' not in infos['cams']['image0']['data_path']:
        #     continue
        if cnt % 10 == 0:
        ...
        pred_boxes = np.array(pred_boxes)[np.array(scores)>args.vis_thred]
        scores = np.array(scores)[np.array(scores)>args.vis_thred]
        if len(pred_boxes) == 0:
        ...
        # image view
        imgs = []
        # if '2070' not in infos['cams'][views[0]]['data_path']:
        #     continue
        for view in views:
        ...
        img = np.zeros((720 * 2 + canva_size * scale_factor, 1280 * 2, 3),
                       dtype=np.uint8)
        img[:720, :, :] = np.concatenate(imgs[:2], axis=1)
        img_back = np.concatenate(
            [imgs[2][:, ::-1, :], imgs[3][:, ::-1, :]],
            axis=1)
        img[720 + canva_size * scale_factor:, :, :] = img_back
