...
def nuscenes_data_prep(root_path, info_prefix, version, max_sweeps=10):
    ...
    for set_ in ['train', 'val']:
        ...
        for fil in ImageSets:
            ...
            calib = Calibration(os.path.join(root_path, 'training', 'calib', fil + '.txt'))
            lidar2ego_translation = np.matmul(cam2psudeo_lidar, np.concatenate((calib.V2C, np.array([[0,0,0,1]]))))
            ...
            for camera_id in calib.CAMERAS:
                ...
                cam['sensor2ego_translation'] = sensor2psudeo_lidar[:3,3]#ego 代表伪激光，原点是camera 0
                cam['sensor2ego_rotation'] = RotateMatirx2unitQ(sensor2psudeo_lidar[:3,:3])
                sensor2lidar = np.linalg.inv(np.matmul(cam2sensor,np.vstack((calib.V2C,np.array([0.,0.,0.,1.])))))
                ...
                cam['ego2global_translation'] = np.array([0.,0.,0.])#随便
            ...
            if show:
                ...
                cv2.imwrite(cams[image_id]['data_path'].split('/')[-1],image)
...
if __name__ == '__main__':
    ...
    root_path = './datasets/zjdata/'
    extra_tag = 'bevdetv2-zjdata'
    show = False
    nuscenes_data_prep(
        root_path=root_path,
